...
def stamp_cover(image, ownership):
    ...
    font_size = int(text_length / len("HARDCOPY") * 2)

    font = ImageFont.truetype("arial.ttf", font_size)  # Change font and size as needed
    length = font.getlength("HARDCOPY")

    text_image = Image.new('RGBA', (int(1.3*length), image_height), (255, 255, 255, 0))
    print(length)
    ...
    rotated_text_image = text_image.rotate(angle, expand=1, center=text_image_center)
    ...
    image.show()  # Display the image with rotated text
